...
from model_aton.datasets import MyHardSingleTripletSelector, SingleTripletDataset
from model_aton.networks import ATONnet, AttentionNet, MyLoss
...
class ATON:
    def __init__(
        ...
        gpu: bool = True,
        device_num: int = 0,
        train_ad_model: bool = True,
        checkpoint_dirpath: str = os.path.join(os.path.dirname(os.getcwd()),"checkpoints"),
    ):
        self.verbose = verbose
        self.checkpoint_dirpath = checkpoint_dirpath
        self.train_ad_model = train_ad_model
    ...
    def interpret_ano(self, ii):
        ...
        attn_net = AttentionNet(
            in_feature=3 * n_linear, n_hidden=int(1.5 * n_linear), out_feature=n_linear
        )
        model = ATONnet(attn_net=attn_net, n_feature=dim, n_linear=n_linear)
        model.to(device)

        if self.train_ad_model:
            ...
            print("-"*50)
            print("Training AD model on training set")
            print("-"*50)
            ...
            if ii == 0:
                print("-"*50)
                print("Loading the AD model from a checkpoint")
                print("-"*50)

            checkpoint_path = get_most_recent_file(self.checkpoint_dirpath)
        ...
        print("-"*50)
        print(f"Producing explanation for anomaly {ii}")
        print("-"*50)
    ...
    def prepare_triplets(self, idx, nbr_indices):
        x = self.x
        y = self.y
        selector = MyHardSingleTripletSelector(
            nbrs_num=self.nbrs_num, rand_num=self.rand_num, nbr_indices=nbr_indices
        )
